-- ============================================================
-- CORREÇÃO: settle_commission_transaction com bank_account_id
-- Necessário para que a trigger de validação ignore o desbalanceamento
-- do ledger quando um banco real é utilizado.
-- ============================================================
...
BEGIN
  ...
  SELECT bank_name, id INTO v_bank_name, v_final_bank_account_id
  FROM bank_accounts
  ...
  IF v_bank_name IS NULL THEN
    ...
    END IF;
    -- Se for financial_account, v_final_bank_account_id fica NULL (correto para ledger balanceado)
  END IF;
  ...
  -- IMPORTANTE: Incluir bank_account_id se for banco real, para passar na validação de trigger
  INSERT INTO financial_transactions (
    ...
    v_legacy_id,
    'confirmed',
    v_final_bank_account_id -- ADICIONADO
  )
  ...
  -- Lançamento de crédito na conta de comissões a receber (reduz o ativo)
  INSERT INTO financial_ledger (transaction_id, account_id, amount, memo)
