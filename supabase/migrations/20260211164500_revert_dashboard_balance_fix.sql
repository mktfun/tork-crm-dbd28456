...
AS $function$
BEGIN
  -- Revertendo para o comportamento original: retorna sempre current_balance da conta
  RETURN COALESCE(
...
CREATE OR REPLACE FUNCTION public.get_bank_transactions(p_bank_account_id uuid, p_start_date date, p_end_date date, p_page integer DEFAULT 1, p_page_size integer DEFAULT 10, p_search text DEFAULT NULL::text, p_status text DEFAULT NULL::text)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
BEGIN
  ...
  -- 1. Calcular totais originais (sem filtro de conciliado)
  SELECT
    COUNT(*)::INTEGER,
  ...
  -- 2. Buscar transações paginadas originals
  SELECT json_agg(
    json_build_object(
      'transactionId', t.id,
