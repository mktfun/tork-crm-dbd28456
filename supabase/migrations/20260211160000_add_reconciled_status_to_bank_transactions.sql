-- Migration: Add is_reconciled to get_bank_transactions
...
BEGIN
  ...
  -- 1. Calcular totais (Receitas e Despesas) CONSIDERANDO FILTRO
  SELECT 
    COUNT(*)::INTEGER,
  ...
  WHERE ft.user_id = v_user_id
    ...
    AND (
      ...
      tx_data.name ILIKE '%' || p_search || '%'
    );

  -- 2. Buscar transações paginadas CONSIDERANDO FILTRO
  ...
  FROM (
    SELECT jsonb_build_object(
      ...
      'is_reconciled', COALESCE(ft.reconciled, false) -- Added field
    ) as tx,
    ft.transaction_date,
    ft.created_at
    FROM financial_transactions ft
