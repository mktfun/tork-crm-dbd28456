-- Issue: Allow consolidated view (p_bank_account_id IS NULL) to include unbanked transactions.
...
BEGIN
  ...
  -- 1. Calcular totais usando ft.type diretamente
  SELECT 
    COUNT(*)::INTEGER,
  ...
  WHERE ft.user_id = v_user_id
    ...
    AND (
      ...
      OR 
      (p_bank_account_id IS NOT NULL AND ft.bank_account_id = p_bank_account_id)
    )
    AND (
      p_search IS NULL OR 
      p_search = '' OR 
  ...
  FROM (
    SELECT jsonb_build_object(
      ...
      'bank_name', COALESCE(ba.bank_name, 'Não Conciliado'),
      'bank_color', ba.color,
