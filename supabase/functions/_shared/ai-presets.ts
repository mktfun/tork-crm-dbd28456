// Shared AI Persona Presets — mapping short IDs to full XML prompts
// This file mirrors the personas from src/components/automation/aiPresets.ts
// for use in edge functions (dispatcher, etc.)
...
// Mapping of short IDs to full XML prompts
const PERSONA_MAP: Record<string, string> = {
  'proactive': VENDEDOR_PROMPT,
  'supportive_sales': AMIGO_PROMPT,
  'technical': TECNICO_PROMPT,
  'supportive': GERAL_PROMPT,
};

/**
 * Resolves a persona ID (e.g. 'proactive') to its full XML prompt.
 * If the value is already a long string (custom prompt), returns it as-is.
 * Returns null if no match and input is a short ID.
 */
export function resolvePersonaPrompt(personaIdOrText: string | null | undefined): string | null {
  if (!personaIdOrText) return null;

  // If it's a known short ID, resolve to full XML
  if (PERSONA_MAP[personaIdOrText]) {
    return PERSONA_MAP[personaIdOrText];
  }

  // If it's already a long prompt (custom), return as-is wrapped in <persona> tags
  if (personaIdOrText.length > 100) {
    return `<persona>\n${personaIdOrText}\n</persona>`;
  }

  // Unknown short ID — return null
  return null;
}
