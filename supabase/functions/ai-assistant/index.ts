...
async function getTechnicalContext(query: string, limit = 5) {
  const supabase = createClient(
    Deno.env.get('SUPABASE_URL')!,
    Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!
  );

  // 1. Gerar o embedding da query (usando a API OpenAI compatível)
  const embeddingResponse = await fetch('https://api.openai.com/v1/embeddings', {
    ...
    body: JSON.stringify({
      input: query,
      model: 'text-embedding-3-small', // Modelo de embedding suportado
    }),
  });
  ...
  // 2. Buscar no banco de dados por similaridade vetorial
  // A função match_documents deve ser criada no Supabase
  const { data: documents, error } = await supabase.rpc('match_documents', {
    query_embedding: embedding,
    match_threshold: 0.78, // Threshold de similaridade
    match_count: limit,
  });
  ...
  // 3. Formatar o contexto para injeção no prompt
  const context = documents.map((doc: any) => {
    return `
--- Fonte: ${doc.source} - ${doc.title} ---
...
1. **Análise Comparativa e Recomendação** — Não apenas compare, mas **recomende** a melhor opção para o perfil do cliente, justificando o "porquê" técnico.
2. **Coberturas Mandatórias vs. Opcionais** — O que é padrão e o que pode ser agregado
3. **Riscos Excluídos Críticos** — Onde a seguradora NÃO paga e o corretor pode errar
4. **Checklist de Documentos** — Listagem completa para submissão à seguradora
...
6. **CITAÇÃO DE FONTE OBRIGATÓRIA**: Sempre que usar informações do RAG, cite a fonte e o título do manual (ex: "Conforme Condições Gerais Zurich Residencial...").
7. **ZERO REDUNDÂNCIA**: Vá direto ao ponto. É PROIBIDO usar frases como "Vou pesquisar", "Um momento", "Com certeza", "Claro!". Comece a resposta com a análise técnica.
</missao_consultoria>
...
serve(async (req) => {
  ...
  generate_report_pdf: async (args, supabase, userId) => {
    ...
    if (!report_type || !data) {
      throw new Error('report_type e data sao obrigatorios');
    }
  ...
  },ceKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!;
    const supabase = createClient(supabaseUrl, supabaseServiceKey);

    // Extract last user message for RAG context
    const lastUserMessage = messages.filter((m: any) => m.role === 'user').pop()?.content || '';
    
    // 1. Adicionar o contexto técnico ao prompt (RAG)
    const technicalContext = await getTechnicalContext(lastUserMessage);
    
    // Build System Prompt with RAG context
    const systemPrompt = await buildSystemPrompt(supabase, userId, lastUserMessage);
    
    // Injetar o contexto RAG no System Prompt
    ...
    console.log(`[PROMPT-BUILD] Length: ${systemPromptWithRAG.length} | Has RAG: ${systemPromptWithRAG.includes('<conhecimento_especializado>')} | Has learned: ${systemPromptWithRAG.includes('<contexto_aprendido>')}`);
