...
def get_embedding(text):
    """Gera embeddings usando o modelo Gemini via API compatível com OpenAI"""
    ...
    payload = {
        "input": text.replace("\n", " "),
        "model": "text-embedding-3-small" # Este modelo é garantido no ambiente
    }
...
def save_to_supabase(chunks, source, category, title):
    ...
    try:
        ...
        data = []
        # Processar os primeiros 50 chunks para cada manual para popular bem a base
        limit = min(len(chunks), 50)
